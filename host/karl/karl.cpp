...
sp <GraphicBuffer> getScreencap() {
  ...
  if (display == NULL) {
    fprintf(stderr, "Unable to get handle for display DEFAULT_DISPLAY_ID\n");
    _exit(1);
  }
  ...
  status_t result = ScreenshotClient::capture(display, Rect(),0 /* reqWidth */, 0 /* reqHeight */, INT32_MIN, INT32_MAX, /* all layers */false, captureOrientation, &outBuffer);
  if (result != NO_ERROR) {
    _exit(1);
  }
	...
	while (clientfd) {
		...
		if (read_len > 0) {
			...
			if(buf[0] == 'w') {
				...
				fprintf(stderr, "======>>>receive height request , poi is [%d,%d]\n",x,y);
				 LOGE("======>>>receive height request , poi is [%d,%d]\n",x,y);
